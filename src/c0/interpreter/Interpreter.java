...
public interface Interpreter {
	...
	//二項演算子
	void binaryOperatorExpressionInit(ExpressionNode expression); //二項演算子の前処理
	...
	void assignExpression(ExpressionNode expression); //代入式
	
	//論理式
	void logicalOperatorExpression(ExpressionNode expression); //条件積演算子, 条件和演算子
	
	//単項式
	void unaryOperatorExpressionInit(ExpressionNode expression); //単項演算子の前処理
	...
	void executeFunctionCall(ExpressionNode callNode); //関数呼び出し（標準関数を含む）
